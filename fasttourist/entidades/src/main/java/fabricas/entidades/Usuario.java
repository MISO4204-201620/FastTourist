...
@NamedQueries({
		...
		@NamedQuery(name = "Usuario.authenticate", query = "SELECT u FROM Usuario u WHERE u.email = :email and u.password=:password and u.activo=1"),
		@NamedQuery(name = "Usuario.findProveedores", query = "SELECT u FROM Usuario u where u.perfil = 1 and u.activo=1"),
		@NamedQuery(name = "Usuario.findSolicitudesBaja", query = "SELECT u FROM Usuario u where u.baja = 1 and u.perfil=1 and u.activo=1") })
